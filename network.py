'''
описывает зоздание и работу с НС
'''
import random
...
class Network:
    def __init__(self, *args):       #число аогументов в соответствующем слое
        self.__nlayers = len(args)  # число слоев
        self.__neuros = args  # число нейронов в каждом слое
        self.__layers = []      #содержит нейроны в кадом слое
        # создаем нейроны в каждом слое
        for i in range(self.__nlayers):
            self.__layers.append([Neuro([], []) for n in range(self.__neuros[i])])  #Neuro([], []) -нейрон без связей
